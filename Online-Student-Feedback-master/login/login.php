<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		while($row = $result->fetch_assoc())
			if(trim($_POST['user']) == trim($row['Roll_No']) && trim($_POST['pass']) == trim($row['Pass']))
				$flag = 1;
	...
	<header>
		...
		<div>
			<form action="login.php" method="post">
				...
				<button type=submit class="btn btn-info">Login</button><a href="registration.php">Create Account</a>
			</form>
		</div>	
	</body>
</html>